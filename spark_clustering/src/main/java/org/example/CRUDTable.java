...
import static org.example.ClusterJob.*;
import static org.example.Constants.WAREHOUSE;
...
//        spark.sql("insert into t1 select 100,'aaa',75,timestamp(16807831), '20230405'");
        ...
        List<Row> rows = spark.sql("select * from t1_ro where uuid in (100, 101, 102)").collectAsList();
        while (true) {
            try {
